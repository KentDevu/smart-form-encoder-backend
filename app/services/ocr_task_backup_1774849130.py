...
from app.services.ocr_groq_extraction import extract_fields_with_groq
from app.services.ocr_preprocessing_hints import enrich_template_with_preprocessing_hints
...
def process_ocr_task(self, form_entry_id: str) -> dict:
    ...
    try:
        ...
        # Step 3: Single Groq extraction call (replaces template-first + AI unified)
        normalized_schema = normalize_template_schema(template.field_schema)
        
        # Construct absolute debug path for checkbox visualization
        # __file__ is at backend/app/services/ocr_task.py, so go up 3 dirs to get backend root
        backend_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        debug_image_path = os.path.join(backend_root, "debug_ocr", form_entry_id)
        
        logger.info(f"[GROQ] Starting single extraction call for {template.name}")
        groq_result = extract_fields_with_groq(
        ...
        if not groq_result:
            logger.error("[GROQ] Extraction failed, falling back to empty result")
            merged_fields: dict[str, dict[str, Any]] = {}
        else:
            # Convert Groq result to merged_fields format
            # CRITICAL: Preserve source from extraction (especially checkbox_detection)
            merged_fields: dict[str, dict[str, Any]] = {
                f["field_name"]: {
                    "value": f.get("value", ""),
                    "confidence": f.get("confidence", 0.5),
                    "source": f.get("source", "groq"),  # Preserve actual source, default to groq
                    "unresolved": False,
                }
                for f in groq_result.get("fields", [])
            }
            
            # Debug: Log which fields have checkbox sources
            checkbox_fields_detected = [name for name, data in merged_fields.items() if data["source"] == "checkbox_detection"]
            if checkbox_fields_detected:
                logger.debug(f"[GROQ-RESULT] Checkbox fields with correct source: {checkbox_fields_detected}")
            
            logger.info(
                f"[GROQ] Extraction complete: {len(merged_fields)} fields, "
                f"avg_confidence: {sum(f['confidence'] for f in merged_fields.values()) / len(merged_fields) if merged_fields else 0:.2f}"
            )
        ...
        for field_name, field_data in merged_fields.items():
            ...
            # Debug: Log checkbox source preservation
            if field_data.get("source") == "checkbox_detection":
                logger.debug(f"[FIELD-SOURCE-MAP] Preserved checkbox_detection source for {field_name}")
        ...
        if template and "dti" in template.name.lower():
            try:
                ...
                # CRITICAL: Preserve source field so DTI rules can check checkbox_detection
                fields_dict = {
                    f["field_name"]: {
                        "value": f.get("ocr_value", ""),
                        "confidence": f.get("confidence", 0.0),
                        "source": f.get("source", ""),  # Preserve source for checkbox detection
                    }
                ...
                # CRITICAL: Preserve original source (especially checkbox_detection)
                source_map = {f["field_name"]: f.get("source", "") for f in mapped_fields}
                
                # Debug: Log which fields have checkbox sources before DTI rules
                checkbox_sources_before = [name for name, source in source_map.items() if source == "checkbox_detection"]
                if checkbox_sources_before:
                    logger.debug(f"[DTI-RULES-PRE] Checkbox sources before rules: {checkbox_sources_before}")
                ...
                mapped_fields = [
                    {
                        ...
                        "source": source_map.get(field_name, "rules"),  # Preserve original source
                    }
                    for field_name, field_data in corrected_fields.items()
                ]
                
                # Debug: Log which fields kept checkbox sources after DTI rules
                checkbox_sources_after = [f["field_name"] for f in mapped_fields if f.get("source") == "checkbox_detection"]
                if checkbox_sources_after:
                    logger.debug(f"[DTI-RULES-POST] Checkbox sources preserved after rules: {checkbox_sources_after}")
                elif checkbox_sources_before:
                    logger.warning(f"[DTI-RULES-POST] WARNING: Lost checkbox sources! Had {len(checkbox_sources_before)}, now have 0")
        ...
        # NOTE: Field validators are optional post-processing. Skipped for now.
